...
// post 方式的/user/login,显示登录页面
router.post('/login', function (req, res) {
    var body = req.body;
    var username = body.username;
    var password = body.password;
    var remember = body.remember;
    var filter = { username: username, password: encrypt(password) };
    // 到数据库中查询
...
router.get('/center', function (req, res) {
    ...
    User.find(filter, fields, function (err, user) {
        if (err) {
            console.log(err);
            res.render('error', { errMsg: '获取数据失败' });
            return;
        }
        if (user.length == 0) {
            res.render('error', { errMsg: '获取数据失败' });
...
router.get('/cut',function(req,res){
    ...
    var x = req.query.x
    var y = req.query.y
    var img = req.query.img; // /xxx.img
    // 剪切图片 ./temp/xxx.jpg
    // 保存路径 .avatar/username/xxx.jpg
    var i = img.substr(1);
